  ...
  return (
    <div className="relative inline-flex flex-col">
      <div className="inline-flex border-2 border-slate-800 rounded text-slate-800 font-mono text-lg divide-x-2 divide-slate-800 bg-white shadow-sm overflow-hidden items-center select-none">
        ...
        {/* Symbol */}
        <div 
          className="px-2 py-1 flex items-center justify-center hover:bg-slate-100 cursor-pointer"
        ...
        {/* Tolerance */}
        <div 
          className="px-2 py-1 flex items-center gap-1 hover:bg-slate-100 cursor-pointer"
        ...
        {/* Datum Primary */}
        <div className={datumClass('A')} onClick={() => handleDatumClick('A')}>A</div>

        {/* Datum Secondary */}
        <div className={`px-2 py-1 flex items-center gap-1 transition-colors hover:bg-slate-100 cursor-pointer ${selectedDatumInFCF === 'B' ? 'bg-blue-100 text-blue-800' : ''}`} onClick={() => handleDatumClick('B')}>
          <span className="font-bold">B</span>
          <span className="inline-flex items-center justify-center border border-slate-800 rounded-full w-5 h-5 text-sm">M</span>
        </div>

        {/* Datum Tertiary */}
        <div className={datumClass('C')} onClick={() => handleDatumClick('C')}>C</div>
      </div>
